...
def users():
    ...
    # Zwróć wszystkich poza zalogowanym
    return jsonify([
...
def test_transfers():
    # Automatyczne generowanie testowych przelewów między użytkownikami
    import random
    results = []
    for _ in range(10):
        ...
        if sender["balance"] >= amount:
            ...
            HoneypotLogger.log_suspicious(f"Test transfer: {amount} EUR {sender['fullname']} -> {recipient['fullname']}")
            results.append({"from": sender["fullname"], "to": recipient["fullname"], "amount": amount})
